...
iSlider.prototype={
    ...
    _bindEvt:function () {
        ...
        handlrElm.addEventListener('mousedown',function (e) {
        ...
        handlrElm.addEventListener('mousemove',function (e) {
            if(!self._mousedown){
                return;
            }
        ...
        handlrElm.addEventListener('mouseup',function (e) {
        ...
        if (this.opts.fullScr || this.opts.preventMove) {
            handlrElm.addEventListener('touchmove', function (e) { e.preventDefault(); }, { passive: false });
        }
    },
...
//                console.info(e)
    ...
    _touchmove_mouse : function (e) {
        ...
        if (this.opts.noslide && this.opts.noslide.indexOf(this.index)>=0) {
            //noslideBack 默认值是false   默认是禁用滑动后 前后都不能再滑动,
            //但是当noslideBack为true时, 禁用了这一页的滑动, 那么往下是划不动了  但是可以往上滑
            if (!this.opts.noslideBack || (e.pageY - this._touchstartY < 0 || e.pageX - this._touchstartX < 0)) {
                return ;
            }
        }
